...
| `CommitLog:FsyncPolicy` | enum | `OnCommit` | live | 01 | `OnCommit` \| `Interval` \| `OsBuffered`. `OnCommit` is the only durable choice; the others trade a bounded window of committed-but-lost transactions for throughput and must say so in their XML docs. **This is the first throughput ceiling you will hit** — see the note below. A live change is a **durability boundary**: everything appended under the outgoing policy is fsynced before the new one takes effect, so leaving `OnCommit` can never publish a buffered LSN as durable (issue #91). |
| `CommitLog:FsyncIntervalMs` | int | `100` | live | 01 | Only read when `FsyncPolicy = Interval`. This is the size of the data-loss window. |
| `CommitLog:GroupCommit` | bool | `true` | live | 07/17 | Batch concurrent commits into one fsync — **honored since phase 17**, which is the concurrent commit path the phase-07 reservation waited for. Improves throughput without weakening durability: a reducer that returns has committed durably, batched or not. Only read under `FsyncPolicy = OnCommit`; `false` restores the phase-01 inline fsync, whose only argument is distrust of the batching, and the fault-injection suite is the answer to that. Like `FsyncPolicy`, a live change is a durability boundary: everything appended under the outgoing setting is fsynced before the new one takes effect. |
...
| `Schema:AllowAdoption` | bool | `false` | restart | 19 | Whether a boot may adopt the running code's schema as the meaning of records already on disk — what happens the first time a directory that predates the shape sidecar boots. Sound exactly when the upgrade rule was followed (boot the old schema once, then change it), which the engine cannot verify, so **the default refuses** and the message names both branches. Turn it on for that one boot and turn it back off; the adoption logs **EventId 1008** when it proceeds. The two errors are not symmetric: refusing costs one boot and one setting, assuming wrongly costs silently mis-read rows found arbitrarily later. A once-per-directory gate by construction — a directory written by a build that has the sidecar always carries one — and adoption over an *empty* directory is a new world naming its first shape, never gated, so a fresh install never meets this. See [MIGRATION.md](MIGRATION.md). |
...
| `Bulk:CreateShards` | bool | `false` | live | #115 | Whether a hub's bulk fan-out may bring destination shards into existence. Off by default: a world generator touching thousands of shard keys would otherwise turn one POST into thousands of shards, originators, and data directories, and reaping an empty one (#112) is a deliberate operator action rather than something that happens on its own — code is revertible where durable directories are not. Off, a batch routing to a shard that does not exist is refused whole **before any engine writes**, naming the shards to pre-declare with `MelangeClusterCoordinator.EnsureShard`. On, the hub creates *and opens* every missing destination up front. Ignored outside `Cluster:Role = Hub`. |
...
| `Subscriptions:MaxRangeSpan` | long | `1024` | live | 03 | Maximum width of a `BETWEEN` predicate. Lets a client stream a ring around itself but not the whole map. Does **not** apply to `col <> 0`, which has no span by construction — that shape is bounded by the row and byte ceilings alone (issue #122). |
...
| `Auth:CaptureClaims` | string[] | *(empty)* | restart | 04 | Claim types copied from a validated token onto `ReducerContext.Claims`, where a reducer can read them. Empty by default: **nothing is captured until a host names what it reads**. An allow-list rather than the whole principal, because captured claims are connection-scoped state held for a session's life, stored against a connect ticket, and — clustered — carried inside every internal identity assertion the gateway mints; an unbounded `ClaimsPrincipal` would make a large token a per-connection cost nobody declared. Matching is ordinal on the type as the token spells it, so an IdP emitting a URI-shaped type wants that URI. A repeated claim keeps every value (`GetValues`); the indexer answers the first. These are validated claims, not client-supplied arguments — which is the point: an application can record who a caller *is* according to the IdP without trusting the client to say so. Restart rather than live, because a connection's claims are fixed at authentication and a live change would apply only to sessions opened after it. |
...
| `Telemetry:SlowReducerMs` | int | `50` | live | 02 | Reducers that exceed this threshold get a span event and a log entry. Pick the number as **"how long may one transaction freeze every other writer"** — a global write-latency budget, not a per-caller one. The number compared against it differs by isolation: under the default `Isolation.Serialized` it is the **whole transaction**, since the write lock covers the whole body and the durability wait after it is latency the caller paid, so a stalled disk behind a trivial body still warns. Under `Isolation.Snapshot` it is the **locked portion** alone — the body ran outside the lock and stalled nobody, so a 500 ms snapshot body does not fire this and should not. The `1003` entry carries both numbers (`LockedMs`, `DurationMs`), the isolation, and `FiredMeasure` naming which one crossed, so a reducer that is slow for its caller without being slow for the world stays visible. |
