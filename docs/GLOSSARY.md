...
**Predicate, not-default** — The `col <> <default>` subscription shape (`<> 0` on an unsigned integer column,
`<> false` on a `bool`): the rows where an indexed column has been set at all. The operand must be the column's *own* default, which is what keeps it an index walk that steps over
