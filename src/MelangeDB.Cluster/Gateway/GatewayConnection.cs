            ...
            case AuthResult next:
                ...
                if (_hub is { IsAlive: true } hub)
                {
                    await hub.SendFrameAsync(new ReauthenticateFrame(_gateway.Hub.MintAssertion(
                        next.Identity, next.IsGuest, next.IsSqlOwner, next.IsBulkOwner, next.TokenExpiresAt, firesLifecycle: true, next.IsBackupOwner,
                        next.CapturedClaims)), ct).ConfigureAwait(false);
                }

                if (_shard is { IsAlive: true } shard)
                {
                    await shard.SendFrameAsync(new ReauthenticateFrame(_gateway.Hub.MintAssertion(
                        next.Identity, next.IsGuest, next.IsSqlOwner, next.IsBulkOwner, next.TokenExpiresAt, firesLifecycle: false, next.IsBackupOwner,
                        next.CapturedClaims)), ct).ConfigureAwait(false);
                }
